...
if(list.isEmpty()){
	out.println("<center><h1>There Is No Pending Orders</h1></center>");
}else{%>
			  <div class="flexslider"><table id="customers">
<tr><th>Order_Id</th><th>Amount</th><th>Order_Date</th><th>User</th></tr>
<%
for(OrderBean b : list){
%>
<tr onclick="window.location='ViewOrders?oid=<%=b.getOid()%>&user=<%=b.getUser()%>'" >
<td><%out.println(b.getOid()); %></td>
<td><%out.println(b.getAmount()); %></td>
<td><%out.println(b.getDate()); %></td>
<td><a href="UpdateUserProfile?email=<%=b.getUser()%>"><%=b.getUser()%></a></td>
</tr>
<%
}}
%>
